...
# ==========================================
# 1. Semantic Categories (Updated from eyes chars.txt)
# ==========================================
DENSE_KANJI = set(['瀟', '憎', '浄', '占', '李', '斗', '狄', '灘', '濾', '鼎', '撼'])

# [Eyes_upper]
EYE_UP_LEFT = set(list("だ灯衍行仍了乍仡乞云伝芸茫忙它佗俐仗なｨｪｵｴﾃﾇﾏﾓfr"))
EYE_UP_CENTER = set(list("不示宍亦兀亢万迩尓禾乏弌弍弐泛夾赱符≡女乍气旡まみてテチｪｫｭｮｰｴｵｻﾁﾆﾓﾕﾖェュョヵ"))
EYE_UP_RIGHT = set(list("豺犾狄勿下卞抃圷圦坏心沁气汽斥拆仔竹刃刈付以雫爿なうかて刈ﾊ､ｧｩｪｫｬｭｱｦｳｵｷｹﾁﾃﾇﾕﾖェュヵヶァ"))

# [Eyes_under]
EYE_LOW_LEFT = set(list("芍弋爪心父戈弌弍弐式汽辷込乂癶廴匕丈叱杙之比仆トヽヾゝゞﾞｬｾﾀﾊﾋﾏﾓ㌧､t"))
...
# ==========================================
# 4. Node 3: Hybrid Generator (Fixed Syntax & Weighted Blending)
# ==========================================
...
class SjisVariableWidthGenerator:
    ...
    def INPUT_TYPES(cls):
        return {"required": {
            ...
            "font_path": (TTF_FILES,),      
            "text_lines": ("INT", {"default": 30, "min": 10, "max": 100, "label": "Text Lines (Resolution)"}),
            "letter_spacing": ("FLOAT", {"default": 0.0, "min": -5.0, "max": 10.0, "step": 0.1, "label": "Letter Spacing"}),
            "placement_method": (["Score-Priority", "Sequential"], {"label": "Placement Algorithm"}),
            
            "y_tolerance": ("INT", {"default": 4, "min": 0, "max": 10, "label": "▶ [BASE] Spatial Y Tolerance"}),
            "y_shift_penalty": ("FLOAT", {"default": 0.3, "min": 0.0, "max": 10.0, "step": 0.1, "label": "▶ [BASE] Y-Shift Penalty"}),
            "phase_congruency_weight": ("FLOAT", {"default": 5.0, "min": 0.0, "max": 10.0, "step": 0.1, "label": "▶ [BASE] Phase Penalty"}),
            "density_penalty_weight": ("FLOAT", {"default": 0.5, "min": 0.0, "max": 10.0, "step": 0.1, "label": "▶ [BASE] Density Penalty"}),
            "missing_penalty_weight": ("FLOAT", {"default": 0.5, "min": 0.0, "max": 10.0, "step": 0.1, "label": "▶ [BASE] Missing Penalty"}),
            "frequency_weight": ("FLOAT", {"default": 2.0, "min": 0.0, "max": 20.0, "step": 0.1, "label": "▶ [BASE] Frequency Bonus"}),
            
            "semantic_roi_bypass": ("BOOLEAN", {"default": True, "label": "🟢 Enable Face/Eye ROI"}),
            "roi_phase_weight": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 10.0, "step": 0.1, "label": "   ↳ [ROI] Phase Penalty"}),
            "roi_density_weight": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 10.0, "step": 0.1, "label": "   ↳ [ROI] Density Penalty"}),
            "roi_missing_weight": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 10.0, "step": 0.1, "label": "   ↳ [ROI] Missing Penalty"}),
            "roi_frequency_weight": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 10.0, "step": 0.1, "label": "   ↳ [ROI] Frequency Bonus"}),
            "roi_tone_weight": ("FLOAT", {"default": 0.7, "min": 0.0, "max": 10.0, "step": 0.1, "label": "   ↳ [ROI] Original Tone Match"}),
        },
        "optional": {
            "opt_roi_mask": ("MASK",),
        }}
    ...
    def load_resources(self, char_list_path, font_path):
        node_dir = os.path.dirname(os.path.abspath(__file__))
        ...
        if (self._char_list_cache is None or self._current_font_path != full_font or self._current_csv_path != full_csv): 
            ...
            if os.path.exists(full_csv):
                try: df = pd.read_csv(full_csv, encoding='cp932', quoting=3)
                except: df = pd.read_csv(full_csv, encoding='utf-8', quoting=3)
                if not df.empty:
                    chars = df['char'].astype(str).tolist() if 'char' in df.columns else df.iloc[:, 1].astype(str).tolist()
                    chars = [c.strip('"') if len(c) > 1 and c.startswith('"') and c.endswith('"') else c for c in chars]
                    max_f = max(df['frequency'].fillna(1).astype(float)) if 'frequency' in df.columns else 1
                    freq_scores = [np.log1p(f) / np.log1p(max_f) for f in df['frequency'].fillna(1).astype(float).tolist()] if 'frequency' in df.columns else [1.0]*len(chars)
        ...
        if getattr(SjisVariableWidthGenerator, '_char_groups_cache', None) is None or SjisVariableWidthGenerator._current_font_path != full_font:
    ...
    def _solve_stripe_sequential(self, scores, char_data, w, spacing, last_ink_x, is_roi_mask):
        ...
        while int(round(cur_x)) <= last_ink_x and int(round(cur_x)) < w:
            ...
            if scores[sx, best_idx] <= 0.0:
                ...
                line += self._get_gap_string(gw, is_l)
                cur_x += gw
                is_l = False
                continue
            ...
            cw = char_info['width']
            line += char_info['char']
            placements.append((char_info['char'], float(cur_x), cw, np.any(is_roi_mask[sx:min(w, sx+cw)])))
    ...
    def _solve_stripe_score_priority(self, score_matrix, char_data_list, w, spacing, last_ink_x, is_roi_mask):
        ...
        for c, x, cw, ir in placements:
            ...
            if gp >= self._dot_width:
                ...
                line += gs
                for gc in gs:
                    cx += self._fw_width if gc == '　' else (self._hw_width if gc == ' ' else self._dot_width)
    ...
    def solve_stripe_hybrid(self, row_img_bin, row_tone, row_cos, row_sin, row_roi, row_roi_weights, char_data_list, spacing, w_p, w_d, w_m, w_f, r_w_p, r_w_d, r_w_m, r_w_f, r_t_w, p_method, y_t, y_s_p):
        ...
        last_x = int(ink_p.max() + 5) if ink_p.size > 0 else 0
        ...
        if cv2.countNonZero(row_img_bin) == 0 and not np.any(is_roi_mask):
        ...
        with torch.no_grad():
            ...
            for cw, g in groups.items():
                ...
                calc = torch.where(g['is_dense'].view(N, 1, 1).bool(), torch.where(is_eye.view(N,1,1).bool() ^ b1.bool(), calc - 99999.0, calc - 100.0), calc)
    ...
    def generate_hybrid_ascii(self, original_image, thinned_image, char_list_path, font_path, text_lines, letter_spacing, placement_method, y_tolerance, y_shift_penalty, phase_congruency_weight, density_penalty_weight, missing_penalty_weight, frequency_weight, semantic_roi_bypass, roi_phase_weight, roi_density_weight, roi_missing_weight, roi_frequency_weight, roi_tone_weight, opt_roi_mask=None):
        res = self.load_resources(char_list_path, font_path)
        ...
        img_tone = cv2.resize(cv2.cvtColor(ori_arr, cv2.COLOR_RGB2GRAY), (target_w, target_h), interpolation=cv2.INTER_AREA)
